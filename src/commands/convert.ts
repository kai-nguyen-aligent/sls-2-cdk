...
import {substituteSSM, restoreServerlessYml} from '../steps/substitute-ssm.js'
...
export default class Convert extends Command {
  ...
  async run(): Promise<void> {
    ...
    for (const servicePath of inputPaths) {
      ...
      const projectOutputDir = isMultiProject
  ...
  private async processProject(
    ...
    if (!serverlessYmlPath) {
      this.error(
        `No serverless.yml or serverless.yaml found in ${servicePath}`,
        {exit: 1},
      )
    }

    try {
      // Step 1: Substitute SSM Parameters
      ...
      // Step 3: Remove Serverless-specific resources
      ...
      // Step 5: Build Lambda environment variable map
